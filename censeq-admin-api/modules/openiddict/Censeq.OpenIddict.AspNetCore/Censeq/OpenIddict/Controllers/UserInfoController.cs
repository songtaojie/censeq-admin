...
public class UserInfoController : CenseqOpenIddictControllerBase
{
    ...
    protected virtual async Task<Dictionary<string, object>?> GetUserInfoClaims()
    {
        ...
        {
            claims[AbpClaimTypes.TenantId] = user.TenantId ?? (object)Guid.Empty;
            claims[OpenIddictConstants.Claims.PreferredUsername] = user.UserName ?? string.Empty;
            claims[OpenIddictConstants.Claims.FamilyName] = user.Surname ?? string.Empty;
            claims[OpenIddictConstants.Claims.GivenName] = user.Name ?? string.Empty;
        }

        if (User.HasScope(OpenIddictConstants.Scopes.Email))
        {
            claims[OpenIddictConstants.Claims.Email] = (object?)await UserManager.GetEmailAsync(user) ?? string.Empty;
        ...
        {
            claims[OpenIddictConstants.Claims.PhoneNumber] = (object?)await UserManager.GetPhoneNumberAsync(user) ?? string.Empty;
