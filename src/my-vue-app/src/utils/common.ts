...
import { verifyUrl } from './validate';
...
export function dateFormat(time: string, format: string) {
	...
	const o: Record<'M+' | 'd+' | 'H+' | 'h+' | 'm+' | 's+' | 'q+' | 'S', number> = {
	...
	for (const k in o) {
		if (new RegExp(`(${k})`).test(format)) {
			const val = o[k as keyof typeof o]; // 👈 关键点在这！
			format = format.replace(RegExp.$1, RegExp.$1.length === 1 ? String(val) : `00${val}`.slice(String(val).length));
		}
	}
	return format;
}
