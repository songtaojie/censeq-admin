...
router.beforeEach(async (to, from, next) => {
	...
	if (excludeRoute.includes(to.path) && !isAuth) {
		...
		if (!isAuth) {
			...
			const isRequestRoutes = themeConfig.value.isRequestRoutes;
			if (routesList.value.length === 0) {
				if (isRequestRoutes) {
					// 后端控制路由：路由数据初始化，防止刷新时丢失
					const initialized = await initBackEndControlRoutes();
					// 解决刷新时，一直跳 404 页面问题，关联问题 No match found for location with path 'xxx'
					// to.query 防止页面刷新时，普通路由带参数时，参数丢失。动态路由（xxx/:id/:name"）isDynamic 无需处理
					if (initialized !== false && storesRoutesList.routesList.length > 0) {
					...
					const initialized = await initFrontEndControlRoutes();
					if (initialized !== false && storesRoutesList.routesList.length > 0) {
						return next({ path: to.path, query: to.query, replace: true });
					}
					NProgress.done();
